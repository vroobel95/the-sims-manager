...
export const LoginPage: React.FC = () => {
    ...
    const handleLogInWithEmailAndPassword = () => {
        signInWithEmailAndPassword(authBasic, email, password)
        .then((userCredential) => {
    ...
    <div className={cx(styles['login-page'])}>
        <div className={cx(styles['login-page__panel'])}>
            <Heading tag={'h2'} className={cx(styles['login-page__panel-title'])}>
                {'Sign In'}
            </Heading>
            <Input label='E-mail' onChange={setEmail}/>
            <Input type={'password'} label='Password' onChange={setPassword}/>
            <Button label='Log in' onClick={handleLogInWithEmailAndPassword}/>
            <Label>or</Label>
            <Button label='Sign in with Google' icon={IconGoogle} onClick={handleLogInWithGoogle}/>
        </div>
    </div>)
}